...
using namespace WPT_NAMESPACE;
	...
	void MiniFlashUI::createWIPParameters (SeqLim &rSeqLim, WIPParameterTool& rTool)
	{
		...
		rTool.createDoubleParameter(WIPDoublePeriod, rSeqLim, "T(D)", "(s or (a.u.))", 5, -150, 150, .05, .5);
		...
		rTool.createLongParameter  (WIPLongChannelID, rSeqLim, "Channel ID (Z)", NULL, -150, 150, 1, 10);

		rTool.createSelectionParameter (WIPSelectionType, rSeqLim, "Actuation mode");
		rTool.addDefaultOption         (WIPSelectionType, sOptionType3,  CONSTANTGRD);
		rTool.addOption                (WIPSelectionType, sOptionType1,  SINUSOIDALE);
		rTool.addOption                (WIPSelectionType, sOptionType2,  SQUAREWAVE);
		rTool.addOption                (WIPSelectionType, sOptionType4,  EXTERNGRD_TRACKACTUATE);
		rTool.addOption                (WIPSelectionType, sOptionType5,  EXTERNGRD_MAXTORQUE);
		rTool.addOption                (WIPSelectionType, sOptionType6,  EXTERNGRD_FB);
		rTool.addOption                (WIPSelectionType, sOptionType7,  EXTERNGRD_PI);
		rTool.addOption                (WIPSelectionType, sOptionType8,  EXTERNGRD_PI_CAPSULE);
		rTool.addOption                (WIPSelectionType, sOptionType9,  EXT_MULTI_ORTHO_ROTORS);
		rTool.addOption                (WIPSelectionType, sOptionType10, EXT_MULTI_PARALLEL_ROTORS);
		rTool.addOption                (WIPSelectionType, sOptionType11, EXTERNGRD_LYAPUNOV_POS);
		rTool.addOption                (WIPSelectionType, sOptionType12, EXTERNGRD_LYAPUNOV_VEL);
		...
		rTool.createDoubleParameter(WIPDoubleK1, rSeqLim, "P", "Gain", 8, -1000000., 1000000., .1, 2);
		rTool.createDoubleParameter(WIPDoubleK2, rSeqLim, "flip angle", "degree", 8, -100., 100., .1, 2);

		rTool.createDoubleParameter(WIPDoubleThetaF1, rSeqLim, "ThetaFinal (X)", "rad", 2, -150, 150, 0.01, 1);
		rTool.createDoubleParameter(WIPDoubleRTDelay, rSeqLim, "RT-Delay", "us", 0, 0, 65000, 1, 1);
		rTool.createDoubleParameter(WIPDoubleDurationEXT, rSeqLim, "Duration FB grad", "(s)", 3, 0, 2, 0.001, 0.1);
	
		rTool.createDoubleParameter(WIPDoubleWakeUpDelay, rSeqLim, "WakeUp delay", "us", 0, 0, 65000, 1, 1);
