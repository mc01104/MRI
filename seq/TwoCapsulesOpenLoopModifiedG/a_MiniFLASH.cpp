...
MiniFlash::MiniFlash()
		...
		, m_sGActuate1_y          ("sGActuate1_y")
		, m_sGActuate2_y          ("sGActuate2_y")
		, m_sGActuate3_y          ("sGActuate3_y")
		, m_sWakeUp             ("sWakeUp")
		, m_sSRF01				("sSRF01")
...
*/
	...
	/*
		George added that for the y-gradient
	*/

	m_sGActuate1_y.setMaxMagnitude(42.9);

	m_sGActuate1_y.setAmplitude(23.0);

	m_sGActuate1_y.setDuration(300 + m_WIPParamTool.getDoubleValue (rMrProt, WIPDblePulseWidth1)*MILLI_TO_MICROSECONDS);
	m_sGActuate1_y.setRampDownTime(300);
	m_sGActuate1_y.setRampUpTime(300);

	if( !( m_sGActuate1_y.prep() ) ||
	   !( m_sGActuate1_y.check() ) )
	   return m_sGActuate1_y.getNLSStatus();

	m_sGActuate2_y.setMaxMagnitude(42.9);
	m_sGActuate2_y.setAmplitude(23.0);

	m_sGActuate2_y.setDuration(300 + m_WIPParamTool.getDoubleValue (rMrProt, WIPDblePulseWidth2)*MILLI_TO_MICROSECONDS);
	m_sGActuate2_y.setRampDownTime(300);
	m_sGActuate2_y.setRampUpTime(300);

	if( !( m_sGActuate2_y.prep() ) ||
	   !( m_sGActuate2_y.check() ) )
	   return m_sGActuate2_y.getNLSStatus();

	m_sGActuate3_y.setMaxMagnitude(42.9);
	m_sGActuate3_y.setAmplitude(23.0);

	m_sGActuate3_y.setDuration(300 + m_WIPParamTool.getDoubleValue (rMrProt, WIPDblePulseWidth3)*MILLI_TO_MICROSECONDS);
	m_sGActuate3_y.setRampDownTime(300);
	m_sGActuate3_y.setRampUpTime(300);

	if( !( m_sGActuate3_y.prep() ) ||
	   !( m_sGActuate3_y.check() ) )
	   return m_sGActuate3_y.getNLSStatus();
								...
								MrProtocolData::SeqExpo &rSeqExpo, long lKernelMode, long /* lSlice */, long /* lPartition */, long lLine)
{
	...
	*/
	...
	if(m_c1 > 0)
	{
		fRTEI ( lT                                                     ,             0,         0,         0,  &m_sGActuate1,   &m_sGActuate1_y,               0,    &m_sWakeUp );
		lT+= m_sGActuate1.getTotalTime() + m_WIPParamTool.getDoubleValue (rMrProt, WIPDbleTOFF)*MILLI_TO_MICROSECONDS ;
		fRTEI ( lT                                                     ,             0,         0,         0,  0,   0,               0,    0 );
	...
	if(m_c2 > 0)
	{
		fRTEI ( lT                                                     ,             0,         0,         0,  &m_sGActuate2,   &m_sGActuate2_y,               0,    &m_sWakeUp );
		lT+= m_sGActuate2.getTotalTime() + m_WIPParamTool.getDoubleValue (rMrProt, WIPDbleTOFF)*MILLI_TO_MICROSECONDS ;
		fRTEI ( lT                                                     ,             0,         0,         0,  0,   0,               0,    0 );
		if(lKernelMode == KERNEL_IMAGE)
			m_c2 -= 1;
