								...
								MrProtocolData::SeqExpo &rSeqExpo, long lKernelMode, long /* lSlice */, long /* lPartition */, long lLine)
{
	...
	double timeIcrement = m_WIPParamTool.getDoubleValue (rMrProt, WIPDoublePeriod)+ 0.0055;
	double totalMomentumToApply = 0.0;
	double totalMomentumImageSeq = 0.0;
	double spoilerAmplitude = 0.0;
	double cosFrequency = m_WIPParamTool.getDoubleValue (rMrProt, WIPDoubleK1);
//cout << "timeIcrement " << timeIcrement << endl;
	if (m_WIPParamTool.getSelectionValue(rMrProt, WIPSelectionType) == INITIALIZE)
	{
		//cout << "timeIcrement " << timeIcrement << endl;

		applyRotMatrix(g_theta);
		m_time_sin += timeIcrement;
		//cout << "cos(10*M_PI*time_sin) " << cos(5*M_PI*time_sin) << endl;
		totalMomentumImageSeq = (m_sGradRO.getMomentumTOT() + m_sGReadDeph.getMomentumTOT())/1000.0;//ms*mT/m
		//cout << "totalMomentumImageSeq " << totalMomentumImageSeq << endl;
		totalMomentumToApply = m_WIPParamTool.getDoubleValue (rMrProt, WIPDoubleAmplitude)*cos(2*cosFrequency*M_PI*m_time_sin) *timeIcrement*1000;
		//cout << "totalMomentumToApply " << totalMomentumToApply << endl;

				
		spoilerAmplitude = (fabs(totalMomentumToApply) - totalMomentumImageSeq)/(m_WIPParamTool.getDoubleValue (rMrProt, WIPDoublePeriod)*1000);

		if((totalMomentumToApply - totalMomentumImageSeq) >= 0)
			spoilerAmplitude = fabs(spoilerAmplitude);

		//cout << "spoilerAmplitude " << spoilerAmplitude << endl;
		if(spoilerAmplitude > 23.0)
			spoilerAmplitude = 23.0;
		if(spoilerAmplitude < -23.0)
			spoilerAmplitude = -23.0;

		if(cos(2*cosFrequency*M_PI*m_time_sin) > 0)
			g_theta = M_PI;
		else
			g_theta = 0;
	
		m_sGActuateX.setAmplitude(spoilerAmplitude);
		m_sGActuateY.setAmplitude(23.0);
		...
		{	
			...
			time_sin += timeIcrement;
			cout << "cos(10*M_PI*time_sin) " << cos(10*M_PI*time_sin) << endl;

			if(cos(10*M_PI*time_sin) > 0)
				g_theta = 0.0;
			else
				g_theta = M_PI;
...
NLS_STATUS MiniFlash::applySETrackingActuationForCapsules(MrProt &rMrProt, long& lT, long lLine)
{
	...
	m_sADCX.getMDH().setKSpaceCentrePartitionNo(0);
	m_sADCX.getMDH().setFreeParameterByIndex(0,1); // 1 means x-axis
	m_sADCX.getMDH().setFreeParameterByIndex(2, (uint16_t)(g_theta*1000));
	m_sADCX.getMDH().setFreeParameterByIndex(3,SIMULTANEOUS); // Interleaved actuation and imaging
...
*/
	...
	m_sADCCom.getMDH().setIceProgramPara(17, (uint16_t) (m_WIPParamTool.getDoubleValue (rMrProt, WIPDoubleK1)*1000) );  // Theta target for third rotor

	m_sADCCom.getMDH().setIceProgramPara(ROTOR_ARM_RADIUS, (uint16_t) (m_WIPParamTool.getDoubleValue (rMrProt, WIPDoublePeriod)*1000));  // Rotor arm radius
